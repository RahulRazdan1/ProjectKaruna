...
class RestController extends \CI_Controller
{
     ...
     * @var array
     */
    protected $allowed_http_methods = ['get', 'delete', 'post', 'put', 'options', 'patch', 'head', 'otp'];
    ...
    protected function _parse_otp()
    {
        $this->_otp_args = $_POST;
    ...
    public function otp($key = null, $xss_clean = null)
    {
        if ($key === null) {
            foreach (new RecursiveIteratorIterator(new RecursiveArrayIterator($this->_otp_args), RecursiveIteratorIterator::CATCH_GET_CHILD) as $key => $value) {
                $this->_otp_args[$key] = $this->_xss_clean($this->_otp_args[$key], $xss_clean);
            }

            return $this->_otp_args;
        }

        return isset($this->_otp_args[$key]) ? $this->_xss_clean($this->_otp_args[$key], $xss_clean) : null;
    }
